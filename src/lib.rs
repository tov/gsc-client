...
impl GscClient {
    ...
    fn save_cookie(&self, response: &reqwest::Response, mut cookie_lock: CookieFile) -> Result<()> {
    ...
    fn send_request(&self, req_builder: reqwest::RequestBuilder) -> Result<reqwest::Response> {
        let cookie = self.load_cookie_file()?;
